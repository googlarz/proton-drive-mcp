...
### Fixed
- **Flag injection via `--password`/`--expiration` on `drive_share_set_url`.** Confirmed live: `sharing set-url ... --password "-x"` makes the real CLI's arg parser treat `-x` as an unknown flag and dump usage instead of setting the password — the same flag-injection class fixed for other params in v1.0.33, just missed here. Added `validateFlagValue()` and applied it in both the MCP dispatch and the CLI companion.
- **`drive_upload` silently disabled thumbnail generation on every upload**, since v1.0.0. `upload()` hardcoded `--skip-thumbnails` unconditionally — never exposed as an option, never documented. Every file uploaded through this tool got no Drive thumbnail, degrading grid/preview views in the Proton apps, with no way to opt out. Removed the hardcoded flag.
- **`photos_list_timeline` with `loadDetails=true` did nothing useful.** The tool description promises "full node metadata" when `loadDetails=true`. Confirmed live that `photo timeline --load-details` returns rich objects (`name`, `mediaType`, `creationTime`, `totalStorageSize`, `photo.captureTime`, `photo.tags`, ...) — but the code discarded everything except `nodeUid`, identical to the cheap default-mode output. `loadDetails=true` paid for a slower, memory-buffered CLI call and threw the result away. Now extracts and returns the full set of fields.
- **`drive_list` didn't escape a literal `/` in an item's name when building its path.** The CLI's own path syntax requires escaping `/` in a name with a backslash. Our `validateName()` prevents *our* tools from creating such names, but an item created by any other Proton Drive client (web, desktop, mobile) can still have a literal `/` in its name. Confirmed live: creating such an item via the raw CLI, then naively joining its name into a path made every path-based tool (`drive_info`, `drive_download`, `drive_move`, `drive_delete`, ...) fail with "Node not found", while the correctly escaped form resolved. `drive_list` now escapes `/` in names before building the path it returns.
- **`drive_invitation_accept`/`drive_invitation_reject` had no flag-injection guard on `uid`**, in both the MCP tool and the CLI. Confirmed live: `invitation accept "-fakeuid"` was misparsed as a CLI flag by the underlying binary ("Unknown option 'f'") instead of a clean rejection. Now validated with `validateFlagValue()`.
- **`drive_download` silently swallowed partial-download failures.** `download()` narrowed the CLI's real `TransferSummary` response down to `{path, localPath, downloaded}`, dropping `skippedItems`/`failedItems` entirely — and the MCP dispatch never checked for failure, unlike `drive_upload`'s equivalent check. A download where some files failed would report success. `DownloadResult` now includes `skipped`/`failed`, and `drive_download` fails the call when `failed > 0`, mirroring `drive_upload`.
- **`drive_read_file`/`drive_write_file` echoed the raw, untrimmed input path** instead of the validated one, inconsistent with every other tool in the codebase (which always echo the validated variable). Minor but real: a path with leading/trailing whitespace would show up unchanged in the response even though the actual operation used the trimmed path.
